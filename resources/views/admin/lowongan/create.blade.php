...
@section('content')
    <div class="max-w-2xl mx-auto bg-white p-6 rounded shadow">
        <h1 class="text-2xl font-bold text-gray-800 mb-4">Tambah Lowongan</h1>

        <form action="{{ url('/admin/lowongan') }}" method="POST" class="space-y-4">
            @csrf
            ...
            <div>
                ...
                <textarea name="deskripsi" rows="4" required class="w-full border px-4 py-2 rounded"></textarea>
            </div>

            <div class="grid md:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Lokasi</label>
                    <input type="text" name="lokasi" required class="w-full border px-4 py-2 rounded">
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Batas Waktu Lamaran</label>
                    <input type="date" name="batas_lamaran" required class="w-full border px-4 py-2 rounded">
                </div>
            </div>
